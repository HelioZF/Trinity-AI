...
- 🧠 **NEXT SESSION: the Phases 0–2 FULL RECAP (user-requested; rule 5) — runs BEFORE Phase 3.** Pacing = Option B, two sittings. **Sitting 1:** Block 0 — *blank-page map*: the user draws the whole pipeline (arXiv → batch tensor) **from memory** (~15 min, NO notes/Codex), annotating each stage with its job + key numbers + `src/` file; Claude grades it against the real machine. Then Block 1 — *concept lightning quiz* (~20 Q across Phases 0–2: UTF-8/code points, BPE train-by-frequency/encode-by-order, specials, the two costs (params∝vocab vs attention∝n²), split/batching/shuffling, manifest pattern, Big O). **Between sittings:** the user handwrites a one-page master summary from memory. **Sitting 2:** Block 2 — *code teach-back + bug hunt* (plant bugs from our real history: str+list gluing, the `seen=set()` overwrite, the `ids[i+1]` bounds check, missing `register_special_tokens` in `load`); Block 3 — *transfer problems* (e.g. switch to cs.CL: what retrains?; vocab→4,096: what ripples?; trace an emoji end-to-end). **Method: test-first — attempt cold; Codex/notes are the answer key AFTER each attempt, never before. Every miss → gap list → re-teach from a different angle → re-test ONLY the gaps at Phase 3 study start.** Weight embeddings, the two costs, n², and batching — load-bearing for Phase 3.
