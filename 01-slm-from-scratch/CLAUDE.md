...
- 🧠 **RECAP (Phases 0–2, rule 5) — Sitting 1 ✅ done 2026-07-31.** Blank-page map graded (frame survived; Phase 2 machinery was foggy) + 20-Q lightning quiz (7✅/10⚠️/3❌). Gap list captured in the Codex **"Burn-in corner"** (Recap chapter). **Handwritten master summary ✅ — strong:** zero-grad, logits, chain-rule reuse, encode-by-creation-order, SEP-as-boundary, specials-atomicity, overfitting all correct from memory. ⚠️ 3 OCR-ambiguous spots the user still must verify on paper in color: (1) "a byte is 8 **bits**" (their recurring error is "bytes"), (2) lengths max **769** / min **51** (OCR read 709/91), (3) the batch-size trade line — throughput + smoother gradients, paid in **activation VRAM** (their one quiz ❌ — they'd blamed attention). **NEXT SESSION: Sitting 2.** Open with a ~10-min **gap-list re-test warm-up** (user-requested — rapid-fire questions, answers after; NOT re-reading), then the homework check (3 colored-pen spots). Then Block 2 — *code teach-back + bug hunt* (plant bugs from our real history: str+list gluing, the `seen=set()` overwrite, the `ids[i+1]` bounds check, missing `register_special_tokens` in `load`); Block 3 — *transfer problems* (switch to cs.CL: what retrains?; vocab→4,096: what ripples?; trace an emoji end-to-end). **Method: test-first — attempt cold, answer key after. Then re-test ONLY the gap list at Phase 3 study start.** Weight embeddings, the two costs, n², and batching — load-bearing for Phase 3.
- ⏭️ **Then: Phase 3 — Transformer Architecture.** **Study first:** Karpathy *"Let's build GPT: from scratch"* → quiz → handwritten summary → build module-by-module (Codex Phase 3 field guide: draft config d_model 384 / 6 heads / 6 layers / ~12 M params). **Open decision at phase start:** context window — sequence max 769 vs avg 317; decide truncate-vs-extend with percentiles. See [`ROADMAP.md`](./ROADMAP.md).
