...
> **📍 You are here: Phase 2 ✅ complete — Phase 3 (Transformer Architecture) is next.** Phase 2 delivered: 2,000-paper cs.LG corpus (manifest-reproducible), tokenizer trained at vocab 2,048 + specials (`SEP=2048, EOT=2049, PAD=2050` → model vocab **2,051**, committed as `models/arxiv_2048.model`), `prepare_data.py` (sequences avg 317 / max 769), and `dataset.py` (`TitleAbstractDataset` + padded `DataLoader`, verified batch `(32, 577)`). See `notebooks/Phase-2-memo.md` and the Codex Phase 2 chapter. **BEFORE Phase 3: the Phases 0–2 full recap** (planned, Option B — see CLAUDE.md Current Status for the block-by-block plan: blank-page map → lightning quiz → handwritten summary → bug hunt → transfer problems; test-first, gap list, re-test). **Then Phase 3 starts with Study:** watch Karpathy's *"Let's build GPT: from scratch"*, then quiz → handwritten summary → build (Codex Phase 3 field guide has the config draft d_model 384 / 6 heads / 6 layers; the context-window size is an open decision — max seq 769 vs avg 317, decide with percentiles). **⚠️ Fresh machine after `git pull`: run `restore_from_manifest()` then `prepare_data.py` to rebuild the gitignored data.**
